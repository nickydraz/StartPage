...
function onLoad() {
    ...
    $(".list").hover(function () {
        $(this).css("box-shadow", "inset 1px 1px 10px #000");
...
function weather() {
    ...
    var sLoc;
    jQuery.when(sLoc = jQuery.getJSON("http://freegeoip.net/json/")).done(function (json) {
        ...
        try {
            $.simpleWeather({
                ...
                , error: function (error) {
                    $("#weather").html('<span>Welcome. Where would you like to go?</span>');
                    Console.log(error);
                }
            }); // end $.simpleWeather
        }
...
function SubmitSearch() {
    //Check for any of the quick patterns
    var quickPattern = /^reddit$|^facebook$|^twitter$|^youtube$|^amazon$|^netflix$|^hulu$|^hbo$|^hbo now$|^hbonow$|^github$|^code school$|^codeschool$|^tfs$|^teamserver$|^team$|^stack$|^stackoverflow$|^stack overflow$|^cloud$|^owncloud$|^ndraz$|^dev$|^digitalocean$|^digital ocean$|^ocean$|^digital$|^php$/;
    var userVal = $('#searchBar').val();
    ...
    if (pattern.test(userVal)) {
        ...
        if (pattern.test(userVal)) {
            $(location).attr("href", userVal);
        }
        else {
            $(location).attr("href", "http://" + userVal);
        }
    }
    else {
        //if nothing else, do a search with DuckDuckGo
        $(location).attr("href", "https://duckduckgo.com?q=" + userVal);
    }
}