...
(function (global, factory) {
    ...
    function day_of_week__handleStrictParse(weekdayName, format, strict) {
        ...
        if (!this._weekdaysParse) {
            ...
            for (i = 0; i < 7; ++i) {
                mom = create_utc__createUTC([2000, 1])
                    .day(i);
    ...
    function checkOverflow(m) {
        ...
        if (a && getParsingFlags(m)
            ...
            if (getParsingFlags(m)
                ._overflowDayOfYear && (overflow < YEAR || overflow > DATE)) {
                overflow = DATE;
            }
            if (getParsingFlags(m)
                ._overflowWeeks && overflow === -1) {
                overflow = WEEK;
            }
            if (getParsingFlags(m)
                ._overflowWeekday && overflow === -1) {
                overflow = WEEKDAY;
            }
    ...
    function cloneWithOffset(input, model) {
        ...
        if (model._isUTC) {
            res = model.clone();
            diff = (isMoment(input) || isDate(input) ? input.valueOf() : local__createLocal(input)
                .valueOf()) - res.valueOf();
    ...
    function moment_calendar__calendar(time, formats) {
        ...
        return this.format(output || this.localeData()
            .calendar(format, this, local__createLocal(now)));
    }
    ...
    function getSetWeekYearHelper(input, week, weekday, dow, doy) {
        var weeksTarget;
        if (input == null) {
            return weekOfYear(this, dow, doy)
                .year;
        } else {
